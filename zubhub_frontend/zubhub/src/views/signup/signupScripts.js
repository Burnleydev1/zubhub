...
export const setLocationWithTelCountry = props => {
  if (vars.iti?.getSelectedCountryData) {
    ...
    if (country_name && props.values.user_location !== country_name) {
...
export const setLabelWidthOfStaticFields = (refs, document, props) => {
  refs.date_of_birth_el.current.childNodes[1].childNodes[0].style.width = `${calculateLabelWidth(
    props.t('signup.inputs.dateOfBirth.label'),
    document,
  )}px`;
  refs.phone_el.current.childNodes[1].childNodes[0].style.width = `${calculateLabelWidth(
    props.t('signup.inputs.phone.label'),
    document,
  )}px`;
};
...
export const validationSchema = Yup.object().shape({
  ...
  dateOfBirth: Yup.date().max(new Date(new Date().getFullYear() - 12, 1, 1), 'oldEnough), 'max').required('required'),
